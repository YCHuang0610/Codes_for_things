...
accessions = ('A2Z669,B5ZC00,P07204_TRBM_HUMAN,P20840_SAG1_YEAST').split(',')
print(accessions)
path = r'C:\Users\11262\PycharmProjects\rosalindproblems\no16.fasta'

handle = open(path, 'w')
for accession in accessions:
    response = urllib.request.urlopen('http://www.uniprot.org/uniprot/%s.fasta' %accession) #urllib.request访问网页
    responseread = response.read().decode('utf-8')
    handle.write(responseread)
handle.close()

prorecords = SeqIO.parse(path,'fasta')
'''
用re编译正则表达式[ST]取任意，[^P]取反
'''
import re
regex = re.compile(r'N(?=[^P][ST][^P])')
for pro in prorecords:
    if re.search(regex,str(pro.seq)):
        print(pro.id)
        for m in re.finditer(regex,str(pro.seq)):
            print(m.start()+1)
